...
class BollingerBreakoutStrategy(BaseStrategy):
    ...
    def __init__(
        self, bb_period: int = 20, bb_std: float = 2.0, volume_filter: bool = True
    ):
        """
    ...
    def calculate_signals(self, data: List[Dict]) -> List[Dict]:
        ...
        for i, d in enumerate(data):
            ...
            if i > 0:
                ...
                if closes[i] > upper[i] and closes[i - 1] <= upper[i - 1] and volume_ok:
                    signal = 1
                ...
                # Short: Preço rompe banda inferior (breakout para baixo)
                    ...
                    closes[i] < lower[i] and closes[i - 1] >= lower[i - 1] and volume_ok
                ):
                    signal = -1
                    band_width = (upper[i] - lower[i]) / middle[i]
                    strength = min(band_width, 1.0)
...
def create_bollinger_breakout_strategy(
    bb_period: int = 20, bb_std: float = 2.0, volume_filter: bool = True
...
STRATEGY_CONFIG = {
    ...
    "default_params": {"bb_period": 20, "bb_std": 2.0, "volume_filter": True},
}
