...
class BacktestEngine:
    ...
    def backtest_signals(
        ...
        try:
            ...
            if len(df) != len(signals):
                logger.error(
                    f"Tamanhos incompatíveis: df={len(df)}, signals={len(signals)}"
                )
                return BacktestEngine._empty_metrics()
    ...
    def walkforward(
        ...
        try:
            ...
            if len(df) < train_size + test_size:
                logger.warning(
                    f"Dados insuficientes para walk-forward: {len(df)} < {train_size + test_size}"
                )
            ...
            for start in range(0, len(df) - train_size - test_size + 1, step_size):
                ...
                try:
                    ...
                    window_metrics = BacktestEngine.backtest_signals(
                        test_df, signals, fee_bps
                    )
            ...
            logger.info(
                f"Walk-forward completed: {windows_processed} windows processed"
            )
    ...
    def _aggregate_walkforward_metrics(metrics_list: List[Dict]) -> Dict:
        ...
        for metric in simple_avg_metrics:
            values = [
                m.get(metric, 0) for m in metrics_list if not np.isnan(m.get(metric, 0))
            ]
    ...
    def optimize_parameters(
        ...
        try:
            ...
            logger.info(
                f"Otimizando {len(param_combinations)} combinações de parâmetros"
            )
            ...
            for i, params in enumerate(param_combinations):
                try:
                    ...
                    if (i + 1) % 10 == 0:
                        logger.info(
                            f"Processadas {i + 1}/{len(param_combinations)} combinações"
                        )
    ...
    data = pd.DataFrame(
        {
            ...
            "high": prices
            * (1 + np.abs(np.random.normal(0, noise_factor * 2, n_bars))),
...
if __name__ == "__main__":
    ...
    wf_metrics = BacktestEngine.walkforward(
        data, simple_ma_strategy, {"period": 20}, train_size=500, test_size=100
    )
