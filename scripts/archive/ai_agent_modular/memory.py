...
class MemoryManager:
    ...
    def __init__(
        self, memory_dir: str, max_experiments: int = 10000, backup_frequency: int = 100
    ):
        """
    ...
    def log_experiment(self, experiment: Dict):
        ...
        try:
            ...
            df_combined.to_parquet(
                self.experiments_file, index=False, compression="snappy"
            )
            ...
            logger.debug(
                f"Experimento registrado: {experiment.get('strategy', 'unknown')}"
            )
    ...
    def _calculate_all_strategies_stats(self, df: pd.DataFrame) -> Dict:
        ...
        for strategy in df["strategy"].unique():
            ...
            stats["strategies"][strategy] = self._calculate_single_strategy_stats(
                strategy_df
            )
        ...
        stats["strategy_ranking"] = sorted(
            strategy_rewards, key=lambda x: x["mean_reward"], reverse=True
        )

        return stats

    def get_best_experiments(
        self, limit: int = 10, metric: str = "reward"
    ) -> pd.DataFrame:
        """
    ...
    def analyze_parameter_impact(
        self, strategy: str, parameter: str, days: int = 30
    ) -> Dict:
        """
        ...
        try:
            ...
            # Análise estatística
            param_df = pd.DataFrame({"param_value": param_values, "reward": rewards})
            ...
            grouped = (
                param_df.groupby("param_value")["reward"]
    ...
    def reset_memory(self):
        ...
        try:
            ...
            if self.experiments_file.exists():
                final_backup = (
                    self.backup_dir
                    / f"final_backup_before_reset_{datetime.now().strftime('%Y%m%d_%H%M%S')}.parquet"
                )
                df = pd.read_parquet(self.experiments_file)
                ...
                info.update(
                    {
                        "total_experiments": len(df),
                        "unique_strategies": (
                            df["strategy"].nunique() if not df.empty else 0
                        ),
                        "file_size_mb": self.experiments_file.stat().st_size
                        / (1024 * 1024),
                        "oldest_experiment": (
                            df["ts"].min()
...
if __name__ == "__main__":
    ...
    for i in range(20):
        ...
        experiment = {
            ...
            "params_json": json.dumps(
                {"param1": np.random.randint(10, 50)}, sort_keys=True
            ),
    ...
    if "strategies" in stats:
        ...
        for strategy, strategy_stats in stats["strategies"].items():
            print(
                f"   {strategy}: {strategy_stats.get('total_experiments', 0)} exp, "
