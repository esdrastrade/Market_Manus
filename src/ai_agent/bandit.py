...
class UCB1Bandit:
    ...
    def ensure_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_mean_reward ON arms(mean_reward DESC)')
    ...
    def update_arm(self, strategy: str, params: Dict, reward: float):
        ...
        if result is None:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, 1, ?, ?, ?, ?)
            ''', (strategy, params_json, reward, reward, 
            ...
            ''', (new_pulls, new_total_reward, new_mean_reward, 
                  datetime.now().timestamp(), datetime.now().isoformat(), arm_id))
            
            logger.info(f"Updated arm {strategy}: pulls={new_pulls}, mean_reward={new_mean_reward:.3f}")
    ...
    def import_arms_data(self, data: Dict):
        ...
        for arm_data in data['arms']:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (strategy, params_json, pulls, total_reward, mean_reward,
                  datetime.now().timestamp(), datetime.now().isoformat()))
...
if __name__ == "__main__":
    ...
    for i, stat in enumerate(stats, 1):
        print(f"   {i}. {stat['strategy']}: {stat['pulls']} pulls, reward={stat['mean_reward']:.3f}")
