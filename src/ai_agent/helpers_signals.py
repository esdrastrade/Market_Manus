...
class SignalHelpers:
    ...
    @staticmethod
    def ema_crossover(df: pd.DataFrame, fast: int, slow: int, 
                     volume_filter: bool = False) -> pd.Series:
        """
        ...
        try:
            ...
            logger.debug(f"EMA Crossover: fast={fast}, slow={slow}, signals={signals.value_counts().to_dict()}")
    ...
    @staticmethod
    def rsi_mean_reversion(df: pd.DataFrame, period: int, lo: float, hi: float,
                          smoothing: int = 3) -> pd.Series:
        """
        ...
        try:
            ...
            if lo >= hi:
                logger.warning(f"Limite inferior ({lo}) deve ser menor que superior ({hi})")
                return pd.Series(0, index=df.index)
            ...
            logger.debug(f"RSI Mean Reversion: period={period}, lo={lo}, hi={hi}, signals={signals.value_counts().to_dict()}")
    ...
    @staticmethod
    def breakout(df: pd.DataFrame, lookback: int, buffer_bps: float,
        ...
        try:
            ...
            logger.debug(f"Breakout: lookback={lookback}, buffer={buffer_bps}bps, signals={signals.value_counts().to_dict()}")
    ...
    @staticmethod
    def bollinger_bands_breakout(df: pd.DataFrame, period: int = 20, std_dev: float = 2.0,
                                volume_filter: bool = True) -> pd.Series:
        """
        ...
        try:
            ...
            logger.debug(f"Bollinger Breakout: period={period}, std={std_dev}, signals={signals.value_counts().to_dict()}")
    ...
    def macd_crossover(df: pd.DataFrame, fast: int = 12, slow: int = 26, signal: int = 9) -> pd.Series:
        ...
        try:
            ...
            logger.debug(f"MACD: fast={fast}, slow={slow}, signal={signal}, signals={signals.value_counts().to_dict()}")
    ...
    def combine_signals(signals_list: list, method: str = 'majority') -> pd.Series:
        ...
        try:
            ...
            logger.debug(f"Sinais combinados ({method}): {combined_signals.value_counts().to_dict()}")
...
if __name__ == "__main__":
    ...
    for name, strategy, params in strategies_to_test:
        ...
        print(f"   {name}: Long={signal_counts.get(1, 0)}, "
    ...
    print(f"   Majority: Long={maj_counts.get(1, 0)}, Short={maj_counts.get(-1, 0)}, Neutro={maj_counts.get(0, 0)}")
    print(f"   Unanimous: Long={una_counts.get(1, 0)}, Short={una_counts.get(-1, 0)}, Neutro={una_counts.get(0, 0)}")
