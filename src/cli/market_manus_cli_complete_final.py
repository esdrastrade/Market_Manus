...
class CapitalTracker:
    ...
    def __init__(self, initial_capital: float, position_size_pct: float = 2.0, 
                 compound_interest: bool = True, max_drawdown_pct: float = 50.0):
    ...
    def check_drawdown_protection(self) -> bool:
        ...
        self.current_drawdown_pct = ((self.max_capital_reached - self.current_capital) / self.max_capital_reached) * 100
...
class BybitConnectivityTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('retCode') == 0:
                    ...
                    results['errors'].append(f"Market data error: {data.get('retMsg', 'Unknown')}")
            else:
                results['errors'].append(f"Market data HTTP error: {response.status_code}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('retCode') == 0 and data.get('result', {}).get('list'):
                    ...
                    results['errors'].append(f"Symbols error: {data.get('retMsg', 'No data')}")
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('retCode') == 0:
                    ...
                    results['errors'].append(f"Auth error: {data.get('retMsg', 'Unknown')}")
            else:
    ...
    def run_full_connectivity_test(self) -> Dict:
        ...
        overall_status = {
            'timestamp': datetime.now().isoformat(),
            'public_api': {
                'status': all([public_results['server_time'], public_results['market_data']]),
            ...
            'overall_health': 'healthy' if public_results['server_time'] and public_results['market_data'] else 'degraded',
            'recommendations': []
        }
        ...
        if not overall_status['public_api']['status']:
            ...
            overall_status['recommendations'].append("Verificar se Bybit API está operacional")
        ...
        if public_results.get('latency_ms', 0) > 1000:
            overall_status['recommendations'].append("Latência alta detectada - considerar VPN")
...
class ReportExporter:
    ...
    def export_trades_csv(self, trades: List[Trade], filename: str = None) -> str:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'timestamp', 'strategy', 'entry_price', 'exit_price',
                'position_size_usd', 'duration_hours', 'pnl_pct', 'pnl_usd',
                'fees_usd', 'net_pnl_usd', 'is_profitable'
            ]
    ...
    def export_capital_history_json(self, capital_history: List[Dict], filename: str = None) -> str:
    ...
    def export_performance_summary_json(self, summary: Dict, filename: str = None) -> str:
...
class TradeSimulator:
    ...
    def simulate_backtest(self, strategy: str, params: Dict = None) -> Dict:
        ...
        return {
            ...
            'total_return': (base['avg_win'] * base['win_rate'] + base['avg_loss'] * (1 - base['win_rate'])) * 100 + random.uniform(-5, 5),
...
def display_connectivity_results(results: Dict):
    ...
    if private['details'].get('errors') and 'não configurados' in private['details']['errors'][0]:
...
class MarketManusCompleteCLI:
    ...
    def setup(self):
        ...
        try:
            ...
            if self.api_key and self.api_secret:
                masked_key = self.api_key[:8] + '...' if len(self.api_key) > 8 else self.api_key
    ...
    def display_startup_status(self):
        ...
        if summary['is_trading_halted']:
            print(f"💰 Capital Manager: ❌ TRADING INTERROMPIDO ({summary['halt_reason']})")
        else:
        ...
        # Modo de operação
        if self.api_key and self.connectivity_status and self.connectivity_status['private_api']['status']:
            print("🎯 Modo de Operação: 🔴 TRADING REAL (API ATIVA)")
            print("   ⚠️ CUIDADO: Operações reais com dinheiro real!")
        elif self.connectivity_status and self.connectivity_status['public_api']['status']:
    ...
    def display_main_menu(self):
        ...
        print("📊 Capital atual: ${:,.2f} | Retorno: {:+.2f}%".format(
            summary['current_capital'], summary['total_return_pct']))
        
        if summary['is_trading_halted']:
            print(f"🛑 TRADING INTERROMPIDO: {summary['halt_reason']}")
        else:
            print(f"🛡️ Proteção ativa: Drawdown máximo {summary['max_drawdown_pct']:.1f}%")
    ...
    def handle_configure_capital(self):
        ...
        print(f"Compound interest: {'Ativo' if summary['compound_interest'] else 'Inativo'}")
    ...
    def _save_capital_config(self, capital: float, position_size: float, 
                           compound: bool, max_drawdown: float):
    ...
    def single_strategy_test(self):
        ...
        try:
            ...
            if 0 <= choice < len(strategy_keys):
                ...
                # Simular backtest
                results = self.trade_simulator.simulate_backtest(strategy_key, strategy['params'])
    ...
    def full_validation(self):
        ...
        for key, strategy in strategies.items():
            ...
            # Resumo rápido
            status = "✅" if results['sharpe_ratio'] >= 1.0 else "⚠️" if results['sharpe_ratio'] >= 0.5 else "❌"
            print(f"   {status} Sharpe: {results['sharpe_ratio']:.2f} | Return: {results['total_return']:.1f}%")
    ...
    def handle_performance_analysis(self):
        ...
        print(f"{return_color} Retorno Total: ${summary['total_return_usd']:+,.2f} ({summary['total_return_pct']:+.2f}%)")
    ...
    def handle_export_reports(self):
        ...
        try:
            if choice == '1':
                filepath = self.report_exporter.export_trades_csv(self.capital_tracker.trades)
                print(f"✅ Trades exportados para: {filepath}")
                
            elif choice == '2':
                filepath = self.report_exporter.export_capital_history_json(self.capital_tracker.capital_history)
                ...
                # Exportar tudo
                trades_file = self.report_exporter.export_trades_csv(self.capital_tracker.trades)
                history_file = self.report_exporter.export_capital_history_json(self.capital_tracker.capital_history)
                summary = self.capital_tracker.get_performance_summary()
                summary_file = self.report_exporter.export_performance_summary_json(summary)
    ...
    def handle_capital_dashboard(self):
        ...
        if summary['current_drawdown_pct'] > 0:
            dd_color = "🟡" if summary['current_drawdown_pct'] < 20 else "🔴"
            print(f"{dd_color} Drawdown Atual:     {summary['current_drawdown_pct']:>12.2f}%")
    ...
    def _simulate_single_trade(self):
        ...
        print(f"{change_color} Mudança: ${result['change_usd']:+.2f} ({result['change_pct']:+.2f}%)")
    ...
    def _simulate_multiple_trades(self):
        ...
        for i in range(num_trades):
            ...
            print(f"   Trade {i+1:2d}: {strategy:<20} {status} ${trade.net_pnl_usd:>+8.2f} | Capital: ${result['new_capital']:>8,.0f}")
        ...
        try:
            while True:
                ...
                if choice == '1':
                    ...
                    print("🚀 Interface completa para trading automatizado de criptoativos!")
                    break
                else:
