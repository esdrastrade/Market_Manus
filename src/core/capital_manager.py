...
class CapitalManager:
    ...
    def __init__(self, config: CapitalConfig):
        ...
        self.capital_evolution.append((datetime.now().isoformat(), self.current_capital))
    
    def calculate_position_size(self, price: float, stop_loss_percent: float = 0.015) -> float:
        """
        ...
        position_size_by_percent = self.current_capital * (self.config.position_size_percent / 100)
        ...
        position_size = min(position_size, self.current_capital * 0.95)  # Máximo 95% do capital
        
        return position_size
    
    def execute_trade(self, entry_price: float, exit_price: float, direction: int,
                     timestamp: str, exit_reason: str, stop_loss_percent: float = 0.015) -> TradeResult:
        """
    ...
    def get_metrics(self) -> Dict:
        ...
        if self.config.compound_interest:
            ...
            final_capital = self.config.initial_capital_usd + sum(t.pnl_usd for t in self.trade_history)
        
        total_return_usd = final_capital - self.config.initial_capital_usd
        total_return_percent = (total_return_usd / self.config.initial_capital_usd) * 100
        ...
        for _, capital in self.capital_evolution:
            ...
            drawdown_percent = (drawdown_usd / peak_capital) * 100 if peak_capital > 0 else 0
        ...
        best_trade_usd = max(t.pnl_usd for t in self.trade_history) if self.trade_history else 0
        worst_trade_usd = min(t.pnl_usd for t in self.trade_history) if self.trade_history else 0
        
        # Tamanho médio das posições
        avg_position_size = sum(t.position_size_usd for t in self.trade_history) / total_trades if total_trades > 0 else 0
        ...
        return {
            ...
            'long_win_rate': len([t for t in long_trades if t.pnl_usd > 0]) / len(long_trades) if long_trades else 0,
            'short_win_rate': len([t for t in short_trades if t.pnl_usd > 0]) / len(short_trades) if short_trades else 0,
    ...
    def _empty_capital_metrics(self) -> Dict:
        ...
        return {
            ...
            'compound_interest': self.config.compound_interest,
            'risk_per_trade_percent': self.config.risk_per_trade_percent,
            'capital_evolution': [(datetime.now().isoformat(), self.config.initial_capital_usd)]
        }
    ...
    @classmethod
    def load_config(cls, filepath: str = 'config/capital_config.json') -> Optional['CapitalManager']:
...
def create_default_capital_config(initial_capital: float = 10000.0) -> CapitalConfig:
    ...
    return CapitalConfig(
        ...
        min_position_size_usd=max(1.0, initial_capital * 0.001),  # Mínimo $1 ou 0.1% do capital
        max_position_size_usd=min(initial_capital * 0.1, 10000.0),  # Máximo 10% do capital ou $10k
...
if __name__ == "__main__":
    ...
    for i, (entry, exit, direction, timestamp, reason) in enumerate(trades, 1):
        ...
        print(f"   Trade {i}: {'+' if result.pnl_usd >= 0 else ''}{result.pnl_usd:.2f} USD "
    ...
    print(f"   📈 Retorno total: ${metrics['total_return_usd']:,.2f} ({metrics['total_return_percent']:.2f}%)")
    print(f"   🎯 Win rate: {metrics['win_rate']:.1%}")
    print(f"   📊 Total trades: {metrics['total_trades']}")
    print(f"   📉 Max drawdown: ${metrics['max_drawdown_usd']:.2f} ({metrics['max_drawdown_percent']:.1f}%)")
