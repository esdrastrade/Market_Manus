...
class StrategyConfig:
    ...
    def __init__(self, name: str, risk_level: str, best_timeframes: List[str], 
                 market_conditions: str, **params):
        self.name = name
    ...
    def update_arm(self, strategy: str, params: Dict, reward: float):
        ...
        if result is None:
            ...
            cursor.execute('''
                ...
                WHERE strategy = ? AND params_json = ?
            ''', (new_pulls, new_total_reward, new_mean_reward, 
                  datetime.now().timestamp(), strategy, params_json))
...
class BacktestEngine:
    ...
    @staticmethod
    def walkforward(df: pd.DataFrame, signal_fn, params: Dict, 
...
class SignalHelpers:
    ...
    @staticmethod
    def rsi_mean_reversion(df: pd.DataFrame, period: int, lo: float, hi: float) -> pd.Series:
...
class AIAgentStrategy(BaseStrategy):
    ...
    def __init__(self, fee_bps: float = 1.5, lam_dd: float = 0.5, 
                 lam_cost: float = 0.1, train: int = 1000, test: int = 250, **kwargs):
    ...
    def _generate_signals_for_strategy(self, df: pd.DataFrame, strategy: str, params: Dict) -> pd.Series:
        ...
        if strategy == 'ema_cross':
            ...
            return SignalHelpers.rsi_mean_reversion(df, params['period'], params['lo'], params['hi'])
        ...
        try:
            ...
            if len(data) >= self.train_size + self.test_size:
                metrics = self.backtest_engine.walkforward(
                    data, self._generate_signals_for_strategy, 
                    {'strategy': strategy, **params},
                    self.train_size, self.test_size, self.fee_bps
                )
            else:
                # Fallback para backtest simples
                metrics = self.backtest_engine.backtest_signals(data, signals, self.fee_bps)
            ...
            logger.info(f"AI Agent experiment logged: reward={reward:.3f}, sharpe={metrics.get('sharpe', 0):.3f}")
...
if __name__ == "__main__":
    ...
    print(f"📊 Distribuição: Long={sum(signals==1)}, Short={sum(signals==-1)}, Neutro={sum(signals==0)}")
    
    # Estatísticas do bandit
    stats = ai_agent.get_bandit_stats()
    print(f"\n🤖 Estatísticas do Bandit:")
    for stat in stats[:3]:  # Top 3
        print(f"   {stat['strategy']}: {stat['pulls']} pulls, reward={stat['mean_reward']:.3f}")
    ...
    if not experiments.empty:
        ...
        for _, exp in experiments.iterrows():
            print(f"   {exp['strategy']}: reward={exp['reward']:.3f}, sharpe={exp['sharpe']:.3f}")
