...
class EMACrossoverStrategy(BaseStrategy):
    ...
    def __init__(self, fast_ema: int = 12, slow_ema: int = 26, volume_filter: bool = True):
        """
    ...
    def calculate_signals(self, data: List[Dict]) -> List[Dict]:
        ...
        """
        if len(data) < max(self.config.params["fast_ema"], self.config.params["slow_ema"]):
...
def create_ema_crossover_strategy(fast_ema: int = 12, slow_ema: int = 26, volume_filter: bool = True) -> EMACrossoverStrategy:
