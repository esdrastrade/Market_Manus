...
class StrategyManager:
    ...
    def combine_strategies(self, strategy_keys: List[str], data: List[Dict]) -> List[Dict]:
        """
        ...
        for i in range(len(data)):
            ...
            combined_signal = self._combine_signals(signals_at_point, strengths_at_point)
            combined_strength = self._combine_strengths(signals_at_point, strengths_at_point)
    ...
    def _combine_strengths(self, signals: List[int], strengths: List[float]) -> float:
        ...
        active_strengths = [
            strength for signal, strength in zip(signals, strengths)
            if signal != 0
        ]
    ...
    def validate_strategy_combination(self, strategy_keys: List[str]) -> Dict[str, Any]:
        ...
        if missing_strategies:
            ...
            validation_result['errors'].append(f"Estratégias não encontradas: {missing_strategies}")
        
        if not existing_strategies:
            validation_result['valid'] = False
            validation_result['errors'].append("Nenhuma estratégia válida na combinação")
        ...
        if len(unique_risks) > 2:
            validation_result['warnings'].append("Combinação com níveis de risco muito diversos")
        
        if 'high' in unique_risks and 'low' in unique_risks:
            validation_result['warnings'].append("Combinação de estratégias de alto e baixo risco")
        ...
        if len(unique_conditions) > 1:
            validation_result['recommendations'].append("Estratégias otimizadas para condições de mercado diferentes")
        
        # Recomendações baseadas no número de estratégias
        if len(existing_strategies) > 3:
            validation_result['warnings'].append("Muitas estratégias podem gerar sinais conflitantes")
        
        if len(existing_strategies) == 1:
            validation_result['recommendations'].append("Considere adicionar estratégia complementar")
            ...
            for i in range(len(strategies)):
                for j in range(i + 1, len(strategies)):
                    for k in range(j + 1, len(strategies)):
                        combinations['triple'].append([strategies[i], strategies[j], strategies[k]])
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for key in strategies:
            ...
            market_condition_distribution[condition] = market_condition_distribution.get(condition, 0) + 1
        ...
        return {
            ...
            'possible_combinations': {
                ...
                'total': len(combinations['single']) + len(combinations['dual']) + len(combinations['triple'])
            }
        }
