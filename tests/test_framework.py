...
class BaseTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        self.logger.debug(f"Teste {self._testMethodName} concluído em {test_duration.total_seconds():.3f}s")
    ...
    def generate_mock_market_data(self, 
        ...
        returns = np.random.normal(0, volatility/np.sqrt(1440), periods)  # 1440 minutos por dia
    ...
    def save_test_config(self, config: Dict[str, Any], filename: str = "test_config.json"):
    ...
    def assert_signal_valid(self, signal: Dict[str, Any]):
        ...
        required_fields = ["timestamp", "symbol", "strategy", "signal", "confidence", "price"]
        
        for field in required_fields:
            self.assertIn(field, signal, f"Campo obrigatório '{field}' não encontrado no sinal")
    ...
    def assert_metrics_valid(self, metrics: Dict[str, Any]):
        ...
        for field in required_fields:
            self.assertIn(field, metrics, f"Campo obrigatório '{field}' não encontrado nas métricas")
    ...
    def assert_alert_valid(self, alert: Dict[str, Any]):
        ...
        for field in required_fields:
            self.assertIn(field, alert, f"Campo obrigatório '{field}' não encontrado no alerta")
        
        # Validar severidade
        valid_severities = ["LOW", "MEDIUM", "HIGH", "CRITICAL"]
        self.assertIn(alert["severity"], valid_severities)
    
    def run_performance_test(self, func, max_execution_time: float = 1.0, iterations: int = 100):
        """
        ...
        # Validar performance
        self.assertLess(stats["mean_time"], max_execution_time, 
                       f"Tempo médio de execução ({stats['mean_time']:.3f}s) excede limite ({max_execution_time}s)")
        
        return stats
    
    def simulate_market_conditions(self, condition_type: str = "normal") -> Dict[str, Any]:
        """
    ...
    def validate_agent_output(self, agent, expected_outputs: List[str]):
        ...
        for output_type in expected_outputs:
            if output_type == "metrics":
                self.assertTrue(agent.save_metrics.called, "Agente deveria ter salvado métricas")
...
class IntegrationTestCase(BaseTestCase):
    ...
    def run_system_integration_test(self, agents: List[Any], duration_seconds: int = 60):
        """
...
if __name__ == "__main__":
    ...
    print("Use as classes BaseTestCase, AgentTestCase e IntegrationTestCase como base para seus testes")
